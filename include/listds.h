...
Node* begin(List* p);			// returns the first node, not header
Node* end(List* p);				// returns the tail which is sentinel node
Node* last(List* p);			// returns the last node
Node* half(List* p);            // returns the node in the middle
Node* find(List* p, int val);	// returns the first node with val found
void clear(List* p);			// frees nodes, not list
bool empty(List* p);			// true if empty, false if no empty
int size(List* p);				// returns the number of nodes in the list
...
// This effectively increases the list size by one.
void push_front(List* p, int val);
...
bool sorted(List* p);				// returns true if sorted by up or down
bool sorted(List* p, int (*comp)(int a, int b));	// using comp fp provided

void push_sorted(List* p, int val); // inserts a node with val sorted
void push_sortedN(List* p, int N);	// inserts N nodes in sorted order, O(n)
...
void bubbleSort(List* p, int(*comp)(int, int) = ascending);
void bubbleSort2(List* p, int(*comp)(int, int) = ascending);
void selectionSort(List* p, int(*comp)(int, int) = ascending);
void quickSort(List* p, int(*comp)(int, int) = ascending);
////////////////////////////////////////////////////////////////////////////
...
void show(List* p, bool all = false);// shows values of nodes in list
