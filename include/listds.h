...
Node* end(List* p);				// returns the tail sentinel node
...
int size(List* p);				// returns the number of nodes
...
void erase(Node* x);			// assumes that x is a valid node
...
// for N times.
void push_backN(List* p, int N, int val = 0);
...
void push_sorted(List* p, int val); // operates in O(n)
...
void show(List* p, bool all = false);
